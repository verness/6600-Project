...
def main():
    ...
    try:
        ...
        meta = np.load(os.path.join(PROCESSED_DIR, "meta.npy"))
        print(f"Loaded dataset: X={X.shape}, y={y.shape}, meta={meta.shape}")
    except FileNotFoundError as e:
        print(f"ERROR: Could not find a required file in {PROCESSED_DIR}\n  {e}")
        return
 
    # Sanity check — all three arrays must have the same number of samples
    assert X.shape[0] == y.shape[0] == meta.shape[0], (
        f"Sample count mismatch: X={X.shape[0]}, y={y.shape[0]}, meta={meta.shape[0]}"
    )
 
    # 2. First split: Separate Training (70%) and Temporary (30%)
    X_train, X_temp, y_train, y_temp, meta_train, meta_temp = train_test_split(
        X, y, meta, test_size=0.3, stratify=y, random_state=42
    )
